...
public:
	TETRA_SERVICE() : Node("tetra_service")
	{
		...
		footprint_publisher = this->create_publisher<geometry_msgs::msg::Pose>("footprint", 10);
		dock_pose_publisher = this->create_publisher<geometry_msgs::msg::PoseStamped>("detected_dock_pose", 10);
		...
		// lintag_subscriber = this->create_subscription<geometry_msgs::msg::PoseStamped>(
		//  	"docking_target_pose", 10, std::bind(&TETRA_SERVICE::LinetagCallback, this, _1));
		...
		delete_map_cmd_srv = create_service<interfaces::srv::DeleteMap>(
        	"delete_map_cmd", 
		std::bind(&TETRA_SERVICE::Delete_Map_Command, this, std::placeholders::_1, std::placeholders::_2));
		...
		Footprint_timer_ = this->create_wall_timer(20ms, std::bind(&TETRA_SERVICE::Footprint_CALC_Timer, this));
	}
	...
	std_msgs::msg::Int32 movebase;
	geometry_msgs::msg::Pose footprint;
	...
	rclcpp::Publisher<std_msgs::msg::Int32>::SharedPtr movebase_publisher;
	rclcpp::Publisher<geometry_msgs::msg::Pose>::SharedPtr footprint_publisher;
	rclcpp::Publisher<geometry_msgs::msg::PoseStamped>::SharedPtr dock_pose_publisher;
	//Subscription ////////////////////////////////////////////////////////////////////////////////////
	rclcpp::Subscription<apriltag_msgs::msg::AprilTagDetectionArray>::SharedPtr apriltag_subscriber;
	rclcpp::Subscription<geometry_msgs::msg::PoseStamped>::SharedPtr lintag_subscriber;
	...
	void LinetagCallback(const geometry_msgs::msg::PoseStamped::SharedPtr msg) 
	{
		if (!tf2_buffer_->canTransform("camera", "docking_target", tf2::TimePointZero)) {
		...
		_pAR_tag_pose.m_iApril_tag_id = 0;
		//TF
		rclcpp::Time now = this->get_clock()->now();
		geometry_msgs::msg::TransformStamped transformStamped;
		transformStamped = tf2_buffer_->lookupTransform("camera", "docking_target", tf2::TimePointZero);
	...
	void ApriltagCallback(const apriltag_msgs::msg::AprilTagDetectionArray::SharedPtr msg) 
	{
		...
		if(size != 0) //detection maker
		{
			...
			// yaw 회전용 쿼터니언 생성 (90도 = M_PI/2 라디안)
			tf2::Quaternion q_yaw;
			q_yaw.setRPY(0, M_PI, 0);  // roll=0, pitch=180, yaw=0

			// 기존 쿼터니언에 yaw 회전 적용
			tf2::Quaternion q_new = q * q_yaw;
			q_new.normalize();  // 정규화 권장

			dock_pose.header.stamp = msg->header.stamp;
			dock_pose.header.frame_id = "camera";
			dock_pose.pose.position.x = _pAR_tag_pose.m_dApril_tag_pose_x;
			dock_pose.pose.position.y = _pAR_tag_pose.m_dApril_tag_pose_y;
			dock_pose.pose.position.z = _pAR_tag_pose.m_dApril_tag_pose_z-0.2;
			dock_pose.pose.orientation.x = q_new.getX();
			dock_pose.pose.orientation.y = q_new.getY();
			dock_pose.pose.orientation.z = q_new.getZ();
			dock_pose.pose.orientation.w = q_new.getW();
			...
			if(_pAR_tag_pose.m_dPositioning_Angle > 0.0)
			{
				...
				if(_pAR_tag_pose.m_transform_old_pose_y > 0)
				{
					_pAR_tag_pose.m_transform_pose_y = _pAR_tag_pose.m_transform_old_pose_y * -0.1; 
				}
				else
				{
					_pAR_tag_pose.m_transform_pose_y = _pAR_tag_pose.m_transform_old_pose_y; 
	...
	void SickCallback(const sensor_msgs::msg::LaserScan::SharedPtr msg)
{
	...
	{
		bool bResult = false;
		if(_pAR_tag_pose.m_transform_pose_x <= 0.85 && _pAR_tag_pose.m_iApril_tag_id != -1) //650mm depart move
		{
			if(_pFlag_Value.m_bFlag_Obstacle_cygbot || _pFlag_Value.m_bFlag_Initialpose)
			{
				cmd.linear.x =  0.0; 
				cmd.angular.z = 0.0;
	...
	void Footprint_CALC_Timer()
	{
		try 
		{
			rclcpp::Time now = this->get_clock()->now();
			geometry_msgs::msg::TransformStamped transformStamped;
			transformStamped = tf2_buffer_->lookupTransform("map", "base_footprint", tf2::TimePointZero);

			footprint.position.x = transformStamped.transform.translation.x;
			footprint.position.y = transformStamped.transform.translation.y;
			footprint.position.z = transformStamped.transform.translation.z;
			footprint.orientation.x = transformStamped.transform.rotation.x;
			footprint.orientation.y = transformStamped.transform.rotation.y;
			footprint.orientation.z = transformStamped.transform.rotation.z;
			footprint.orientation.w = transformStamped.transform.rotation.w;
			footprint_publisher->publish(footprint);

			//RCLCPP_INFO(this->get_logger(),"_pGoal_pose.goal_quarterX: %f", _pGoal_pose.goal_quarterX);
			//RCLCPP_INFO(this->get_logger(),"_pGoal_pose.goal_quarterY: %f", _pGoal_pose.goal_quarterY);
			//RCLCPP_INFO(this->get_logger(),"_pGoal_pose.goal_quarterZ: %f", _pGoal_pose.goal_quarterZ);
			//RCLCPP_INFO(this->get_logger(),"_pGoal_pose.goal_quarterW: %f", _pGoal_pose.goal_quarterW);

			//RCLCPP_INFO(this->get_logger(),"footprint.orientation.x: %f", footprint.orientation.x);
			//RCLCPP_INFO(this->get_logger(),"footprint.orientation.y: %f", footprint.orientation.y);
			//RCLCPP_INFO(this->get_logger(),"footprint.orientation.z: %f", footprint.orientation.z);
			//RCLCPP_INFO(this->get_logger(),"footprint.orientation.w: %f", footprint.orientation.w);


			} 
		catch (tf2::TransformException &ex) 
		{
			// RCLCPP_WARN(this->get_logger(), "Could not transform map to footprint: %s", ex.what());
		}
		...
		if(bOn)
		{
			...
			{	
				...
				if(m_fdistance > 0.65)
				{
					...
					printf("cmd.linear.x ->: %.5f \n", cmd.linear.x);
					printf("cmd.angular.z ->: %.5f \n", cmd.angular.z);
					cmd_vel_publisher->publish(cmd);
			...
			{
				printf("[ChargingStation_tracking] No Marker, Rotation Movement !!! %d, %d\n", _pAR_tag_pose.m_iApril_tag_id , marker_id);
				cmd.angular.z = Rotation_Movement(); //0.1;
				cmd_vel_publisher->publish(cmd);

				if(m_iNoMarker_cnt > 8000) //retry timeout!!
				{
					m_iNoMarker_cnt = 0;
					cmd.linear.x =  0.0; 
					cmd.angular.z = 0.0;
	...
	{ 
		...
		m_fdistance = sqrt(_pAR_tag_pose.m_transform_old_pose_x * _pAR_tag_pose.m_transform_old_pose_x + _pAR_tag_pose.m_transform_old_pose_y * _pAR_tag_pose.m_transform_old_pose_y);
		//printf("master_distance ->: %.5f \n", m_fdistance);
		if(_pAR_tag_pose.m_target_yaw <= 0.0174533/1.5 && _pAR_tag_pose.m_target_yaw >= -0.0174533/1.5) //+- 1.0deg
		{
			m_iDocking_CommandMode = 4;
			bResult = true;
			return bResult;
		...
		if(_pAR_tag_pose.m_target_yaw > 0) //[++dir] 
		{
			if(m_iRotation_cnt <= 100)
			{
				...
				{
					//rotation move
					cmd.linear.x = -1.0 * m_fdistance * 0.1;
					cmd.angular.z = -1.0 * _pAR_tag_pose.m_target_yaw * 2.8;
					cmd_vel_publisher->publish(cmd);
					m_iRotation_cnt ++;
				}
	...
	bool ChargingStation_tracking2(int marker_id)
	{
		...
		else
		{
			...
			cmd.angular.z = 0.0;
			cmd_vel_publisher->publish(cmd);
			printf("[ChargingStation_tracking2] No Marker 2! \n");
		...
		const std::shared_ptr<interfaces::srv::GotoLocation2::Response> response)
	{
		...
		{
			RCLCPP_INFO(get_logger(), "Go to Depart Move !!");
			m_iDocking_CommandMode = 10; //Depart Move
			bResult = true;
