...
public:
	TETRA() : Node("tetra")
	{
		...
		cmd_vel_subscriber = this->create_subscription<geometry_msgs::msg::Twist>("cmd_vel_nav2", rclcpp::SensorDataQoS(), std::bind(&TETRA::velCallback, this, _1));
	...
	void SetMoveCommand(double fLinear_vel, double fAngular_vel)
	{
		...
		dssp_rs232_drv_module_set_velocity2(iData1, iData2, &m_dX_pos, &m_dY_pos, &m_dTheta, &m_dVelocity_l, &m_dVelocity_a, &m_bumper_data, &m_emg_state);
...
int main(int argc, char * argv[])
{
	...
	while (rclcpp::ok() && !stop_requested)
	{
		...
		// if(!bPosition_mode_flag&& m_left_error_code != 48 || m_right_error_code != 48)
		// {
